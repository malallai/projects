...
class UserSql extends Sql {
    ...
    public function getLastUsers($count) {
        try {
            $request = self::runList("SELECT * FROM users ORDER BY id DESC LIMIT ?",  array($count), PDO::FETCH_ASSOC);
            return $request;
        } catch (SqlException $e) {
            Snackbar::sendSnack($e->getMessage());
            return false;
    ...
    public function getUserHomeDetails($userId) {
        try {
            $request = self::run("SELECT users.username, count(user_id) FROM users LEFT JOIN posts AS posts ON posts.user_id = users.id WHERE users.id = ?", array($userId));
            return $request;
